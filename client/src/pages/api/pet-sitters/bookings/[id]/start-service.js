...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Check if the current status is "waiting for service"
    if (bookingCheck.status !== "waiting for service") {
      return res.status(400).json({
        message:
          "Cannot start service. Booking must be in 'waiting for service' status.",
      });
    }

    // Update booking status to "in service"
