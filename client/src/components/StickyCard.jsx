    ...
    <div>
      <div className="bg-white p-6 rounded-lg shadow-md sm:mb-10 flex flex-col items-center text-center">
        ...
        <div className="flex flex-row gap-5 mb-1 items-center">
          <div>
            <p className="text-black text-2xl mb-5">{sitter.name}</p>
          </div>
          <div>
            <p className="text-green-400 text-xl mb-5">
        ...
        <p className=" text-green-500 font-medium text-xl mb-5 flex flex-row items-center">
          {Array.from({ length: Math.floor(sitter.average_rating ?? 0) }).map(
            (_, i) => (
              <svg
                key={i}
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-5">
          {sitter.pet_type?.map((type, i) => {
            ...
            switch (type) {
              ...
              case "Mouse":
                style =
                  "bg-purple-100 text-purple-400 border border-purple-400";
                break;
              case "Turtle":
                style =
                  "bg-yellow-100 text-yellow-400 border border-yellow-400";
                break;
              case "Snake":
