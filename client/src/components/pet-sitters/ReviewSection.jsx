  ...
  useEffect(() => {
    const fetchData = async () => {
      const [
        ...
        supabase.from("pet_sitter").select("average_rating").eq("user_id", sitterId).single(),
        supabase.from("reviews").select("*", { count: "exact", head: true }).eq("sitter_id", sitterId),
        supabase.from("reviews").select("*").eq("sitter_id", sitterId).order("created_at", { ascending: false }),
      ...
      if (reviewListError || usersError) {
        console.error("Error fetching reviews or users:", reviewListError || usersError);
        return;
      }
    ...
    <div className="w-[90%] bg-stone-200 p-10 rounded-tl-[4rem] flex flex-col gap-6 mx-auto">
      <div className="bg-white flex flex-col  sm:flex-row rounded-tl-[4rem] rounded-tr-[1rem] sm:rounded-tr-[1rem] rounded-br-[1rem] sm:rounded-br-[1rem] rounded-bl-[1rem] sm:rounded-tl-full  sm:rounded-bl-full overflow-hidden p-2">
        ...
        <div className="bg-black text-white mx-3 my-3  w-25 h-25 sm:w-38 sm:h-30 flex flex-col justify-center items-center rounded-tl-full rounded-tr-full rounded-bl-full">
          <span className="text-5xl font-thin">{averageRating !== null ? Math.round(averageRating) : '-'}</span>
        ...
        <div className="flex flex-col justify-center px-6 py-4">
          ...
          <div className="flex flex-wrap gap-2">
                <button
                    ...
                    activeRatingFilter === null ? 'bg-white text-orange-500' : 'bg-orange-500 text-white border-gray-300'
                    }`}
                >
                    All Reviews
                </button>
                {[5, 4, 3, 2, 1].map(val => (
                    ...
                    className={`px-3 py-1 border rounded text-sm flex items-center gap-1 cursor-pointer ${
                        val === activeRatingFilter ? 'bg-orange-500 text-white' : 'bg-white border-gray-300'
                    }`}
                    >
                    {val} {renderStars(val)}
                    </button>
      ...
      <div className="flex flex-col gap-6 mx-5 my-3">
        {paginated.map((review) => (
          <div key={review.id} className="border-t border-gray-300 pt-4 flex flex-col sm:flex-row gap-4">
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex justify-center items-center gap-2">
            ...
            >
              <Image src="/assets/arrow1.png" alt="prev" width={50} height={50} />
            </button>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-xl ${
                  page === currentPage ? 'bg-orange-200 text-orange-500' : 'bg-gray-100 text-gray-500'
                }`}
              >
                {page}
              </button>
            ...
            >
              <Image src="/assets/arrow2.png" alt="next" width={50} height={50} />
            </button>
          </div>
        )}
      </div>
