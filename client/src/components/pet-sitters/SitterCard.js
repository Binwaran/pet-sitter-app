...
const SitterCard = ({ sitter, pet_sitter, isSelected, onClick }) => {
  ...
  const handleCardClick = () => {
    if (data?.user_id) {
      ...
      router.push(`/pet-sitter/${data.user_id}`);
    } else {
  ...
  if (!data) { 
    console.warn("Sitter data is unexpectedly null for SitterCard. Rendering a placeholder.");
    return (
      <div
      ...
      >
        <p className="text-gray-500">ข้อมูล Sitter ไม่พร้อมใช้งาน (Fallback Placeholder)</p>
      </div>
    );
  }
  ...
  let displayImageUrl = data.gallery_image_url
    ...
    : data.imageUrl;
  if (!displayImageUrl || typeof displayImageUrl !== 'string' || displayImageUrl.trim() === '') {
    displayImageUrl = '/assets/placeholder-image.jpg';
  }

  return (
    <div
      className={`flex flex-col md:flex-row gap-4 p-4 bg-white rounded-xl shadow-sm hover:shadow-md transition cursor-pointer relative ${isSelected ? 'border-2 border-orange-500' : ''}`}
    ...
    >
      ...
      <div className="relative w-full h-48 md:w-40 md:h-32 rounded-md overflow-hidden bg-gray-100">
        <Image
          src={displayImageUrl}
          alt={`ภาพร้านของ ${data?.trade_name || data?.name || 'ไม่ระบุชื่อร้าน'}`}
          fill
      ...
      <div className="flex-1 flex flex-col justify-between">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
          <div className="flex items-center">
            ...
            <div className="relative w-10 h-10 rounded-full overflow-hidden bg-gray-100 mr-2">
              <Image
                src={data?.users?.profile_image_url || '/assets/placeholder-profile.jpg'}
            ...
            <div>
              <h2 className="text-base font-semibold text-gray-800">{data?.trade_name || data?.name || 'ชื่อร้านยังไม่ระบุ'}</h2>
              <p className="text-sm text-gray-500">By {data?.users?.name || 'ไม่ระบุชื่อ'}</p>
            </div>
          </div>
        </div>
        {/* ที่อยู่ */}
        {data?.province && (
          <div className="mt-2 flex items-center gap-2">
            ...
            <p className="text-sm text-gray-600">{data?.district || 'ไม่ระบุเขต/อำเภอ'}</p>
          </div>
        )}
        {/* Tags */}
