...
function useReviewData(sitterId) {
  ...
  const fetchReviews = useCallback(async () => {
    if (!sitterId) {
      setState((prev) => ({
        ...prev,
        loading: false,
        error: "No pet sitter ID provided",
      }));
      return;
    }
  ...
  const deleteReview = useCallback(async (reviewId) => {
    try {
      await axios.delete(`/api/admin/pet-sitters/reviews?id=${reviewId}`);
