...
const BookingSummaryCard = ({ bookingDetails, sitterTradeName }) => {
  ...
  const sitterHouse =
    sitterTradeName ||
    bookingDetails.trade_name ||
    bookingDetails.sitterHouse ||
    bookingDetails.sitterHouseName ||
    "";
  ...
  if (bookingDetails.start_time && bookingDetails.end_time) {
    timeRange = `${formatTimeHM(bookingDetails.start_time)} - ${formatTimeHM(
  ...
  const total = bookingDetails.total
    ? `${parseFloat(bookingDetails.total).toLocaleString(undefined, {
