...
export default function PetSitterMap() {
 ...
 useEffect(() => {
    if (selectedSitter && swiperRef.current && Array.isArray(sitters) && sitters.length > 0) {
      const index = sitters.findIndex(sitter => sitter.id === selectedSitter.id);
  ...
  if (loading) {
    return <div className="flex justify-center items-center min-h-screen">Loading sitters...</div>;
  }
        ...
        <div className="absolute bottom-0 left-0 w-full z-10 py-0">
          ...
          modules= {[]}
            ...
            breakpoints={{
              ...
              1024: { slidesPerView: 1.5, centeredSlides: true, spaceBetween: 20 },
            }}
          >
             {Array.isArray(sitters) && sitters.map((sitter, index) => (
              <SwiperSlide key={sitter?.id || sitter?.user_id || `sitter-${index}`}> 
             <SitterCard
              sitter={sitter}
