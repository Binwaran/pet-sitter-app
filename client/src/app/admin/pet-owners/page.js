...
// กำหนด STATUS_MAP สำหรับ owner (ปรับตามระบบคุณ ถ้ามี status)
const STATUS_MAP = {
  active: {
    text: "Active",
    color: "text-[#1CCD83]",
...
const usePetOwners = () => {
  ...
  const fetchOwners = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get("/api/owners/search");
      // สมมติ res.data.data คือ array ของ owner
  ...
  const filteredOwners = useMemo(() => {
    return owners.filter((o) => {
      ...
      const matchSearch =
        !keyword ||
        (o.full_name && o.full_name.toLowerCase().includes(keyword)) ||
        (o.email && o.email.toLowerCase().includes(keyword));
      return matchStatus && matchSearch;
    });
  }, [owners, selectedStatus, search]);

  const sortedOwners = useMemo(() => {
    if (filteredOwners.length === 0) return [];

    const sortable = [...filteredOwners];
  ...
  const totalPages = useMemo(
    () => Math.ceil(sortedOwners.length / rowsPerPage),
    [sortedOwners.length, rowsPerPage]
  );

  const paginatedOwners = useMemo(
    () => sortedOwners.slice((page - 1) * rowsPerPage, page * rowsPerPage),
    [sortedOwners, page, rowsPerPage]
  );
...
export default function AdminPetOwnersPage() {
  ...
  const handleOwnerClick = useCallback(
    (ownerId) => {
      router.push(`/admin/pet-owners/${ownerId}`, {
        scroll: false,
      });
    },
    [router]
          ...
          <div className="flex-1 flex flex-col gap-6 pt-10 px-12 pb-20 max-w-full w-full transition-all duration-300 relative h-full min-w-0">
            <div className="flex flex-col items-center lg:flex-row sm:justify-between gap-6 w-full">
              ...
              <div className="flex flex-col lg:flex-row gap-4 w-full sm:justify-end">
                ...
                {/* ถ้า owner ไม่มี status ให้ซ่อน StatusDropdown ได้ */}
                <StatusDropdown
                  ...
                  {loading ? (
                    ...
                    paginatedOwners.map((owner) => (
                      <tr
                        ...
                        onClick={() => handleOwnerClick(owner.user_id)}
                      >
